...
class Power4Activity : AppCompatActivity() {
    private val PREFS_NAME = "power4_prefs"
    private val KEY_POWER4_NAME = "power4_name"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val powerLabel = findViewById<TextView>(R.id.power4)
        val accPowerTextView = findViewById<TextView>(R.id.acc_power4)
        val accCostTextView = findViewById<TextView>(R.id.acc_cost4)
        val outCountTextView = findViewById<TextView>(R.id.out_count4) // ✅ 대기전력차단횟수

        // 대기전력차단횟수 기본값 0 설정
        outCountTextView.text = "0회" // 대기전력차단횟수 기본값

        val prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.getString(KEY_POWER4_NAME, null)
            ...
            AlertDialog.Builder(this)
                ...
                .setPositiveButton("확인") { _, _ ->
                    ...
                    if (newName.isNotEmpty()) {
                        ...
                        prefs.edit {
                            putString(KEY_POWER4_NAME, newName)
                        }
                    }
                }
                .show()
        ...
        // ✅ 차트 설정
        lineChart = findViewById(R.id.line_chart4)

        // ✅ Firebase logs에서 최신 10개 current 값 가져와 그래프에 그리기
        val logsRef = Firebase.database.getReference("logs4")
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (entries.isNotEmpty()) {
                    ChartSetup.setupLineChart(
                        chart = lineChart,
                        entries = entries,
                        label = getString(R.string.power_concent4),
        ...
        // ✅ 전력값 계산 및 표시
        val currentRef = Firebase.database.getReference("current4")
        currentRef.child("current").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val real_power = getString(R.string.real_power4, power)
                findViewById<TextView>(R.id.real_power4).text = real_power

                val ratePerKWh = 121.32
                val usageKWh = (power / 1000.0) * (1.0 / 60.0)
                val cost = usageKWh * ratePerKWh

                val real_cost = getString(R.string.real_cost4, cost)
                findViewById<TextView>(R.id.real_cost4).text = real_cost
            }
